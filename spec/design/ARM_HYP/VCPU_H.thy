...
where
"dissociateVCPUTCB tcbPtr vcpuPtr\<equiv> (do
...
where
"associateVCPUTCB tcbPtr vcpuPtr\<equiv> (do
    tcbVCPU \<leftarrow> archThreadGet atcbVCPUPtr tcbPtr;
    (case tcbVCPU of
        Some ptr \<Rightarrow>   dissociateVCPUTCB tcbPtr ptr
      | _ \<Rightarrow>   return ()
      );
    vcpu \<leftarrow> getObject vcpuPtr;
    (case (vcpuTCBPtr vcpu) of
          Some ptr \<Rightarrow>   dissociateVCPUTCB ptr vcpuPtr
        | _ \<Rightarrow>   return ()
        );
...
where
"invokeVCPUWriteReg vcpuPtr reg val \<equiv> writeVCPUReg vcpuPtr reg val"
...
"performARMVCPUInvocation x0\<equiv> (case x0 of
  ...
  | (VCPUInjectIRQ vcpuPtr _ _ _ _ _) \<Rightarrow>    error []
  )"
...
"decodeARMVCPUInvocation label args capIndex slot x4 extraCaps\<equiv> (let cap = x4 in
  if isVCPUCap cap then  
    (case invocationType label of
        ...
        | ArchInvocationLabel ARMVCPUInjectIRQ \<Rightarrow>   error []

...
locale Tcb_AI_1 =
  ...
  "\<And>t. \<lbrace>\<lambda>(s::'state_ext state). invs s\<rbrace> arch_tcb_sanitise_condition t
       \<lbrace>\<lambda>b s. invs s\<rbrace>"
  assumes arch_tcb_sanitise_condition_tcb_at[wp]:
  "\<And>t a. \<lbrace>\<lambda>(s::'state_ext state). tcb_at a s\<rbrace> arch_tcb_sanitise_condition t
       \<lbrace>\<lambda>b s. tcb_at a s\<rbrace>"
  assumes arch_tcb_sanitise_condition_ex_nonz_cap_to[wp]:
  "\<And>t a. \<lbrace>\<lambda>(s::'state_ext state). ex_nonz_cap_to a s\<rbrace> arch_tcb_sanitise_condition t
